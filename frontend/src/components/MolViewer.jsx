...
export default function MolViewer({ pdbData, status }) {
    ...
    const applyStyle = useCallback((style) => {
        ...
        try {
            // Apply specified styles carefully
            const styleMap = {
            ...
            console.error('Failed to change 3Dmol visualization style', err);
        }
    }, []);
    ...
    useEffect(() => {
        ...
        import('3dmol/build/3Dmol-min.js').then(($3Dmol) => {
            ...
            // Clear previous viewer if reinitializing
    ...
    return (
        <div className="mol-viewer__container" ref={!pdbData ? null : undefined} role="region" aria-label="3D Protein Structure Viewer">
            {!pdbData ? (
                <div className="mol-viewer__placeholder">
                    <div className="mol-viewer__placeholder-icon" aria-hidden="true">
                        {status === 'processing' ? (
                ...
                <>
                    <div
                        ...
                        aria-label="Interactive 3D molecule viewer widget"
                    />
                    {/* Visualization style selector */}
                    <div className="mol-viewer__style-bar" role="group" aria-label="Visualization styles">
                        {VIZ_STYLES.map((s) => (
                            <button
                                key={s.key}
                                className={`mol-viewer__style-btn${vizStyle === s.key ? ' mol-viewer__style-btn--active' : ''}`}
                                aria-pressed={vizStyle === s.key}
                    ...
                    <div className="mol-viewer__overlay-controls" role="group" aria-label="Viewer controls">
                        <button
                            className="mol-viewer__overlay-btn"
                            title="Fullscreen"
                            aria-label="Enter fullscreen viewer mode"
                        ...
                        <button
                            className="mol-viewer__overlay-btn"
                            title="Reset view"
                            aria-label="Reset 3D view and zoom"
                            onClick={() => {
                                if (viewerRef.current) {
                        ...
                        >
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><circle cx="11" cy="11" r="8" /><line x1="21" y1="21" x2="16.65" y2="16.65" /><line x1="11" y1="8" x2="11" y2="14" /><line x1="8" y1="11" x2="14" y2="11" /></svg>
                        </button>
                    </div>
                </>
            )}
